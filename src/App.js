  ...
  return {
    ...
    icons: {
      ...
      svgTemplate(val, path) {
        return `<svg class="${val}" xmlns="http://www.w3.org/2000/svg" :fill="dark ? 'black' : 'white'" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">${path}</svg>`;
      },
      moon(val) {
    ...
    theme() {
      const isDark = !('color-theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches;
  ...
  Alpine.data('observeSections', () => ({
    ...
    observeSections() {
      ...
      headers.forEach(header => {
        header.innerHTML = [...header.textContent].map(char => `<div class="inline-block">${char === ' ' ? '&nbsp;' : char}</div>`).join('');
      });

      const animateItems = (items, yValue, duration, ease, stagger) => {
        gsap.fromTo(items, { opacity: 0, scale: 0, y: yValue }, { opacity: 1, scale: 1, y: 0, duration, ease, stagger });
      };
        ...
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            if (entry.target.tagName === 'H1') {
              animateItems(entry.target.querySelectorAll('div'), 90, 2, "elastic.out(1.2, 0.5)", 0.03);
            } else {
              animateItems(entry.target.querySelectorAll('[data-section="animate"]'), 50, 1, "power4.out", 0.1);
            }
          }
        });
      }, { threshold: 0.5 });
